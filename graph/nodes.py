...
``execute_queries`` (Node 6) is now fully implemented: after the same
``state.error`` / ``state.guardrail_status`` gating as the other stages it
opens one read-only connection (``db.connect.connect_readonly``), executes
``state.sql_main``, then every companion in ``state.sql_companions`` in dict
order -- companions run even when the main query returned zero rows, because
each exclusion count is independently meaningful for disclosure. A
database-level exception never crashes the graph: it becomes ``state.error =
"query_execution_failed:main"`` for the main query or
``"query_execution_failed:<rule>"`` for a companion, while a companion result
that fails the strict count-shape rule (exactly one row and exactly one
column; a count of 0 is normal -- failure is zero rows or extra rows/columns,
never the count value itself) sets ``"query_result_shape_invalid:<rule>"`` and
stops companion execution immediately. Write-back is atomic, mirroring
``validate_guardrails``: ``state.main_results`` and every companion's
``status`` / ``excluded_count`` accumulate in local variables and are written
only after the main query and all companions succeed, so any failure returns
the state exactly as it arrived.
...
``assemble_disclosures`` (Node 6.5) is now fully implemented: after a
passthrough gate (``state.error`` set, ``guardrail_status`` not ``"passed"``,
or ``state.main_results`` still ``None``) it hard-fails on any
``applicable_rules``/``sql_companions`` inconsistency for the three exclusion
rules (``disclosure_assembly_inconsistency:<rule>``), then deterministically
converts every disclosure source -- fired exclusion rules with their companion
counts, the ``NET_VS_GROSS`` direct filter, query assumptions, and
main/companion truncations -- into the unified ``Disclosure`` list in fixed
order.
