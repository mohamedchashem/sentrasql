...
Most functions are still stubs that return ``state`` unchanged. The one real
implementation so far is ``compile_sql`` (Node 4), for its base case plus all
four policy rules. The base case (``state.applicable_rules`` empty) builds
