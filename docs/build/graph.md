...
## assemble_answer (Node 7) — Complete

The second and final LLM call. Uses a templating/reference-substitution architecture (not free generation) — the model outputs a structured segment list (`{type: "text", content}` / `{type: "ref", key}`), never raw numbers directly, via a Pydantic discriminated union enforced at parse time as a first layer, plus a required, absolute digit-in-text scan in `validate_segments()` as the real backstop (schema shape alone doesn't prevent a number appearing in literal text — proven true for Node 2's enum finding, deliberately re-applied here).

**Reference dictionary** (`build_reference_dict`, `graph/reference_dict.py`): built fresh per call from result values (scalar `result.<metric>`, or grouped `result.total` + positional per-row keys `result.rows.<ordinal>.<column>`) and resolved filter values (only when their `present` flag is true). Formatting (currency, integers, dates) decided once in code. `display.top_n` and similar fixed parameters are also routed through the dictionary, so the no-digits-in-text rule has zero exceptions.

**`result.total` for grouped queries** is computed in `compile_sql` (Node 4), not derived post-guardrail — an earlier draft that computed it inside `execute_queries` by mutating the already-approved `sql_main` AST was rejected in design review as a guardrail-bypass security gap (DESIGN_LOG.md §20, corrected).

**Rendering**, three explicit, always-ordered steps: intro (from validated segments), table (grouped-query case only, plain text — not Markdown, since the delivery surface's Markdown support isn't confirmed — deterministic, never touched by the model, reusing the exact same pre-formatted values as the intro so prose and table can never disagree on a number), disclosures (verbatim, always, never model-composed or reordered).

**Retry policy**: one retry on any failure — LLM exception, segment validation failure (missing ref key or digit-in-text), or a caught rendering exception (`render_error:<ExceptionClass>`) — all three failure types are retry-eligible, since rendering happens inside the retry loop, not after it. Retry context includes all collected failure reasons from the prior attempt as structural pointers only (segment index, missing key name) — never the model's actual generated content, so a future reason-taxonomy addition can't accidentally feed fabricated output back to the model on the fix-it call. `state.answer_generation_retried: bool` records occurrence visibly. On final failure, `state.error` holds the primary reason and `state.error_reasons: list[str]` holds every reason from the final attempt specifically (not the first attempt's) — structured, not string-concatenated, for reliable eval-harness assertions.

196 tests total. This closes the last LLM-dependent node in the graph — all eight node functions (including `handle_error`, still pending) are now either complete or trivial.


## Post-Milestone Fix Cycle: Four Issues Found in First Real End-to-End Run

The first genuine end-to-end run of the compiled graph (all eight nodes, real DeepSeek API, real database) surfaced four real issues invisible to the 204 mocked/isolated tests that existed at the time — all four traced to real root causes and fixed:

1. **`AnswerSegments` schema rejected by DeepSeek's strict endpoint.** Root cause: Pydantic's `Field(discriminator=...)` emits an OpenAPI-only `discriminator`/`mapping` keyword that DeepSeek's strict validator rejects (orphaned pointers after LangChain's schema-inlining step). Fixed by flattening to a plain `TextSegment | RefSegment` union with `type` required and `extra="forbid"` on both branches (closing a pre-existing silent-field-drop gap that was previously masked by the discriminator). A first fix attempt (plain union alone) was live-tested and found still-insufficient — LangChain's `dereference_refs`-based tool conversion (used by the real `with_structured_output`/`bind_tools` path) was confirmed, through source tracing and live A/B testing, to already fully inline the schema correctly; the earlier failed re-test had used an isolated schema fragment that bypassed this real conversion path. The real, live `get_answer_model()`-bound call now returns HTTP 200 with a correctly parsed response.

2. **Uncaught crash on a NULL scalar aggregate.** SQLite's scalar `SUM`/`AVG` over zero matching rows return one row with a `NULL` value, not zero rows — violating `GraphState`'s documented "empty list means no data" contract and crashing `reference_dict`'s currency formatter outside any retry envelope. Fixed at three points: `execute_queries` normalizes a NULL scalar aggregate to `main_results = []`; a new compile-time gate rejects `MIN`/`MAX` over the nullable `customer_id` column (the one case where a grouped total could also go NULL while rows exist); and `assemble_answer`'s setup phase is now wrapped so any unexpected exception becomes a terminal, non-retried error instead of crashing the graph.

3. **Missing `assemble_answer` → `handle_error` graph edge.** `assemble_answer`'s failure path set `state.error` but had no conditional edge routing to `handle_error` — the graph dead-ended at `END` with `final_answer = None`. Fixed with a `_route_after_answer` conditional edge mirroring the existing execution/guardrail routing pattern. Proven via a new graph-topology test class (`test_build_graph.py`) — the first tests in this project to invoke the actual wired-together compiled graph rather than individual node functions; the fix's test explicitly fails against the old wiring.
